...
export default class extends React.Component {
  ...
  getDetialProduct = async () => {
    ...
    try {
      ...
      if (resultRes.product.KeySEO) {
        ptotosNew.push({
          html: `<iframe src="https://www.youtube.com/embed/${resultRes.product.KeySEO}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`,
        });
      }
      ...
      this.setState({
        activeImage: { ...ptotosNew[0], index: 0 },
  ...
  orderSubmit = () => {
    ...
    if (!infoUser) {
      ...
      const { arrProduct, quantity, isAdd } = this.state;
      const self = this;
  ...
  render() {
    ...
    const settings = {
      ...
      renderCenterRightControls: ({ nextDisabled, nextSlide }) => null,
      afterSlide: (i) => {
        this.setState({
          activeImage: {
            ...photos[i],
    ...
    return (
      ...
      >
        ...
        <div className="page-render no-bg p-0">
          <div className="page-shop no-bg">
            <div className="page-shop__detail">
              ...
              {!statusLoading && (
                <>
                  <div className="position-relative page-shop__detail-img">
                    <Carousel
                      {...{ ...settings, slideIndex: activeImage?.index || 0 }}
                    >
                      {photos &&
                        photos.map((photo, index) => (
                          <div
                            key={index}
                            onClick={() => this.standaloneDark.open(index + 1)}
                          >
                            {photo.url && (
                              <img
                                className="w-100"
                            ...
                            {photo.html && (
                              <div
                                className="iframe-chid d--f ai--c jc--c h-100"
                                style={{
                                  aspectRatio: 1,
                                  pointerEvents: "none",
                                }}
                    ...
                    <div className="count">
                      {activeImage?.index + 1}/{photos && photos.length}
                    </div>
                  </div>
                  {/* <div
                    className="page-shop__detail-img"
                    onClick={() =>
                      this.standaloneDark.open(activeImage?.index + 1)
                    }
                  >
                    {activeImage?.url && (
                      <img
                    ...
                    {activeImage.html && (
                      <div
                        ...
                        dangerouslySetInnerHTML={{ __html: activeImage.html }}
                      ></div>
                    )}
                    <div className="count">
                      {activeImage?.index + 1}/{photos && photos.length}
                  ...
                  >
                    {photos &&
                      photos.map((photo, index) => (
                        <div
                          className="w-80px min-w-80px max-w-80px mr-10px border bz-bb"
                          style={{
                            opacity: activeImage?.index === index ? 1 : "0.5",
                          }}
                          key={index}
                          onClick={() => {
                            this.setState({
        ...
        <Sheet
          ...
          opened={sheetRoseOpened}
          onSheetClosed={() => this.closeSheetRose()}
          swipeToClose
          swipeToStep
          backdrop
        >
          <div className="sheet-modal-swipe-step">
            ...
            {arrProductCurrent && (
              <div className="sheet-swipe-product__content p-15px">
                <table className="data-table card">
                  <tbody>
                    {arrProductCurrent?.DIEM && arrProductCurrent?.DIEM > 0 ? (
                      <tr>
                        <td className="label-cell">Thanh toán từ điểm</td>
                        <td className="numeric-cell">
                          {arrProductCurrent?.DIEM}%
                        </td>
                      </tr>
                    ) : (
                      ""
                    )}
                    {arrProductCurrent?.TIEN && arrProductCurrent?.TIEN > 0 ? (
                      <tr>
                        <td className="label-cell">Thanh toán từ thẻ tiền</td>
                        <td className="numeric-cell">
                          {arrProductCurrent?.TIEN}%
                        </td>
                      </tr>
                    ) : (
                      ""
                    )}
                    {arrProductCurrent?.BonusSaleJSON &&
                      JSON.parse(arrProductCurrent?.BonusSaleJSON)
                        .filter((x) => x.Salary)
                        .reverse()
                        .slice(0, 1)
                        .map((sale, index) => (
                          <tr key={index}>
                            <td className="label-cell">
                              Tích luỹ {sale.Level.replaceAll("F0", "mua hàng")}
                            </td>
                            <td className="numeric-cell">
                              {sale?.Salary || 0}%
                            </td>
            ...
            <div className="sheet-swipe-product__content">
              ...
              <div className="sheet-pay-body">
                ...
                <div className="sheet-pay-body__btn">
                  ...
                  >
                    <span>{isAdd ? "Thêm vào Giỏ hàng" : "Mua hàng ngay"}</span>
        ...
        <Toolbar tabbar position="bottom">
          <div className="page-toolbar">
            ...
            arrCombos.length === 1 ? (
              ...
              <div className="page-toolbar__order d--f">
                <div
                  className="h-100 d--f ai--c jc--c text-white"
                  style={{
                    background: "#00bfa5",
                    width: "110px",
                    fontSize: "30px",
                  }}
                  onClick={() => this.openSheetRose()}
                >
