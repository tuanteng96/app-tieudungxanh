...
export default class extends React.Component {
  ...
  componentDidMount() {
    ...
    if (infoUser) {
      this.setState({
        ...
        SenderPhone: infoUser?.MobilePhone,
        PID: infoUser?.ProvinceID || "", //2
        PTitle: infoUser?.ProvinceTitle || "", //Hà Giang
        DID: infoUser?.DistrictID || "",
        DTitle: infoUser?.DistrictTitle || "",
        SubDID: infoUser?.WardID || "",
        SubDTitle: infoUser?.WardTitle || "",
      });
    }
    this.getOrder();
  }
  ...
  handleSubmit = () => {
    ...
    if (
      ...
      toast.error("Bạn cần cập nhập đầy đủ thông tin nhận hàng.");
      return;
    }
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          if (data.errors && data.errors.length > 0) {
            ...
            if (Number(order?.ToPayClient) > 0) {
              this.$f7router.navigate(
                "/pay-success/" +
                  data.order.ID +
                  `/?money=${order?.ToPayClient}`
              );
            } else {
            ...
            if (
              ...
              let newUser = {
                ...infoUser,
                DistrictTitle: DTitle,
                DistrictID: DID,
                ProvinceID: PID,
                ProvinceTitle: PTitle,
                WardID: SubDID,
                WardTitle: SubDTitle,
              };
    ...
    return (
      ...
      >
            ...
            <div className="page-pay-1">
              <div className="page-pay__information">
                ...
                >
                  <div className="f--1">
                    ...
                    >
                      ...
                      <span>
                        (+84) {SenderPhone && SenderPhone.substring(1)}
                      </span>
                    </div>
                    <div className="address mt-5px">
                      {SubDTitle} - {DTitle} - {PTitle}
                    </div>
                    <div className="address mt-5px">{SenderAddress}</div>
                  </div>
                  <div
                    className="d--f ai--c jc--e"
                    style={{
                      width: "40px",
                      fontSize: "24px",
                      color: "#999",
                ...
                {(!SenderAddress ||
                  ...
                  !SubDID) && (
                  <div
                    className="text-danger pl-25px mt-8px"
                    onClick={() => this.setState({ opened: true })}
                  >
                    Bạn cần cập nhập đầy đủ thông tin nhận hàng.
                  </div>
                )}
              ...
              {!isLoading && (
                <>
                  ...
                  <div className="bg-white mt-4px">
                    <div className="d--f jc--sb p-15px border-bottom">
                      <div>Tổng tiền</div>
                      <div className="fw-600">
                        {formatPriceVietnamese(order?.TotalValue)}
                      </div>
                    </div>
                    {order?.DIEM ? (
                      <div className="d--f jc--sb p-12px border-bottom">
                        <div>Thanh toán từ điểm</div>
                        <div>
                          {formatPoint(order?.DIEM)} (
                          {formatPriceVietnamese(order?.DIEM)})
                        </div>
                      </div>
                    ) : (
                      ""
                    ...
                    {order?.TIEN ? (
                      <div className="d--f jc--sb p-12px border-bottom">
                        <div>Thanh toán từ thẻ tiền</div>
                        <div>{formatPriceVietnamese(order?.TIEN)}</div>
                      </div>
                    ) : (
                      ""
                    )}

                    <div className="d--f jc--sb p-12px">
                      ...
                      >
                        {formatPriceVietnamese(order?.ToPayClient)}
                      </div>
                    </div>
                  </div>
                </>
                  ...
                  <div className="title">
                    ...
                    <span>
                      {formatPriceVietnamese(
                        order && Math.abs(order?.ToPayClient)
                      )}
                      <b>₫</b>
                    </span>
                  </div>
        ...
        <UpdateSender
          ...
          opened={opened}
          onSheetClosed={this.onSheetClosed}
          onSubmit={(values) => {
            this.setState({
              ...values,
              PID: values?.PID?.ID || "",
              PTitle: values?.PID?.Title || "",
              DID: values?.DID?.ID || "",
              DTitle: values?.DID?.Title || "",
              SubDID: values?.SubDID?.ID || "",
              SubDTitle: values?.SubDID?.Title || "",
              opened: false,
            });
          }}
        />
