...
export default class ProductList extends React.Component {
  ...
  componentDidMount() {
    this.getDataList("794,10106", "1", 4, this.props?.tag || "", "");
  }
  ...
  getDataList = (ID, pi, ps, tag, keys) => {
    ...
    ShopDataService.getList(ID, pi, ps, tag, keys, stockid, tag)
      .then((response) => {
            ...
            {arrCateList && arrCateList.length > 0 && (
              <div className="home-page__product">
                <div className="head">
                  <h5>
                    <Link href="/shop/794/">
                      {this?.props?.Title || "Sản phẩm hot"} <i className="las la-angle-right"></i>
                    </Link>
                  </h5>
                </div>
                <div className="body">
