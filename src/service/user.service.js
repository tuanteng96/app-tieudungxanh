...
class UserService {
  ...
  register({
    ...
    return http.get(
      `/app/index.aspx?Fn=${fullname}&Phone=${phone}&NewPWD=${encodeURIComponent(
        password
      )}&cmd=reg&ByStock=${stock}&USN=${phone}&Gender=${gender}&Address=${address}&FParentID=${FParentID}&birth=${birth}`
    );
  }
  ...
  getFtree(data) {
    return http.get(`/api/v3/ftree@get?MemberID=${data.MemberID}`);
  }
  getForder(data) {
    return http.post(`/api/v3/forder@get`, JSON.stringify(data));
  }
  updateFtree(data) {
    return http.post(`/api/v3/Ftree@MemberUpdate`, JSON.stringify(data));
  }
  drawMoney(data) {
    return http.post(`/api/v3/memberf25@draw`, JSON.stringify(data));
  }
  getCitys(pid = 0) {
    return http.get(`/api/v3/region25@getp?pid=${pid}`)
  }
  getDistrict(did) {
    return http.get(`/api/v3/region25@getd?pid=${did}`)
  }
  getWards(pid, did) {
    return http.get(`/api/v3/region25@getw?did=${did}&pid=${pid}`)
  }
}
